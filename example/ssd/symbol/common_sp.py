...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
	...
	loc_weight = mx.symbol.Variable(name = 'loc_weight')
	loc_bias = mx.symbol.Variable(name="loc_bias",
			init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})
			
	cls_weight = mx.symbol.Variable(name = 'cls_weight')
	cls_bias = mx.symbol.Variable(name="cls_bias",
			init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})	
	...
	for k, from_layer in enumerate(from_layers):
		...
		loc_pred = mx.symbol.Convolution(data=from_layer, weight = loc_weight, bias=loc_bias, kernel=(3,3), \
		...
		# bias = mx.symbol.Variable(name="{}_cls_pred_conv_bias".format(from_name),
			# init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})
		cls_pred = mx.symbol.Convolution(data=from_layer, weight = cls_weight, bias = cls_bias, kernel=(3,3), \
