...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        # bias = mx.symbol.Variable(name="{}_cls_pred_conv_bias".format(from_name),
            # init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})
        prior_prob = 0.01
        bias_ = np.zeros((num_classes), dtype=np.float32)
        bias_[0] = np.log((num_classes - 1) * (1 - prior_prob) / (prior_prob))
        # bias_ = np.vstack(
            # [bias_ for _ in range(num_anchors)]
        # )
        # bias_ = bias_.reshape((num_cls_pred))
	
        # bias = mx.symbol.Variable(name="{}_cls_pred_conv_bias".format(from_name),
            # init=mx.init.Constant(bias_.tolist()), attr={'__lr_mult__': '2.0'})
