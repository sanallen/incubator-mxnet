...
def get_symbol(num_classes = 78, loss = 'ctc', seq_len = 24, dtype = 'float32', **kwargs):
    ...
    logits_b_c_s = mx.sym.Pooling(data = x_all, kernel = (1,24), stride = (1, 1),  pool_type = 'avg', 
    ...
    fc = mx.sym.FullyConnected(data = logits, num_hidden = num_classes+1, no_bias = False, name = 'classifier')
    arg_shape, out_shapes, aux_shapes = fc.infer_shape(data=(128,3,94,24))
    print('fc:',out_shapes)

    if loss:
        # Training mode, add loss
        return _add_ctc_loss(fc, seq_len, 79, loss)

    else:
        # Inference mode, add softmax
        return mx.sym.softmax(data=fc, name='softmax')
    

if __name__ == '__main__':
    net = get_symbol(num_classes = 78, loss='')
    mx.viz.plot_network(net, shape={"data" : (1, 3, 94, 24)}, node_attrs={"shape":'rect',"fixedsize":'false'}).view('lprnet_fc')