...
def pelee_unit(data, filter_num_branch1a, filter_num_branch1b, ratio, stride, name, bn_mom = 0.0):
    ...
    squeeze = mx.sym.Pooling(data=branch1b_relu, global_pool=True, kernel=(7, 7), pool_type='avg', name=name + '/squeeze')
    squeeze = mx.symbol.Flatten(data=squeeze, name=name + '/flatten')
    excitation = mx.symbol.FullyConnected(data=squeeze, num_hidden=int(filter_num_branch1b*ratio), name=name + '/excitation1')
    excitation = mx.sym.Activation(data=excitation, act_type='relu', name=name + '/excitation1/relu')
    excitation = mx.symbol.FullyConnected(data=excitation, num_hidden=filter_num_branch1b, name=name + '/excitation2')
    excitation = mx.sym.Activation(data=excitation, act_type='sigmoid', name=name + '/excitation2/sigmoid')
    squeeze_excitation = mx.symbol.broadcast_mul(branch1b_relu, mx.symbol.reshape(data=excitation, shape=(-1, filter_num_branch1b, 1, 1)))
    
    concat = mx.sym.concat(data, squeeze_excitation, dim = 1, name = name+'/concat')
    return concat
...
def peleenet(units, num_stages, filter_list_branch1a, filter_list_branch1b, filter_list_transition, ratio_list, num_classes, 
    ...
    for i in range(num_stages):
        ...
        for j in range(units[i]):
            dense_layer = pelee_unit(data = dense_layer, filter_num_branch1a = filter_list_branch1a[i], 
                filter_num_branch1b = filter_list_branch1b[i], ratio = ratio_list[0], stride = (1, 1), name = 'stage%d_%d' % (i+1, j+1))
...
if __name__ == '__main__':
    ...
    mx.viz.plot_network(net, shape={"data":(1, 3, 224, 224)}, node_attrs={"shape":'rect',"fixedsize":'false'}).render('se_peleenet')